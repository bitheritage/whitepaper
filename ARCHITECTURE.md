...
The WebAuthn passkey (ECDSA P-256) is used **exclusively for authentication** — signing challenges during login. It is
never used for encryption or key agreement.
...
During registration and each subsequent authentication, the app evaluates the WebAuthn PRF extension to derive the
...
6. The private key is held in memory only for the duration of the session and is never written to disk, local storage,
...
3. Performs X25519 key agreement between the ephemeral private key and the recipient's public key to produce a shared
   secret.
...
_Only the recipient, possessing the X25519 private key (re-derived from their authenticator's PRF secret), can perform
the reverse key agreement to derive the same shared secret and decrypt the data._

### Layer 2: Server-Side Encryption

The Encryption Service derives a unique per-secret key for each encryption operation, ensuring that a single key never
protects more than one secret:
...
3. Encryption Service encrypts the Layer 1 ciphertext with ChaCha20-Poly1305 using the per-secret key and a random
   nonce.
...
The server master key is the root of trust for all Layer 2 encryption. It is managed through an external **Secrets
Manager** (e.g., HashiCorp Vault, AWS Secrets Manager, or GCP Secret Manager):
...
- **Access control:** Only the Encryption Service's identity is authorized to read the master key from the Secrets
  Manager. All access is audited.
...
- `secrets` has a unique constraint on `(owner_id, inheritor_id, title)` — one secret per owner-inheritor-title
  combination.
- `users.encryption_public_key` stores the X25519 public key derived from the authenticator's PRF extension during
  registration. This key is used by counterparties to encrypt secrets for this user.
- `secrets.ephemeral_public_key` and `secrets.nonce` store the per-secret cryptographic parameters required for Layer 1
  decryption.
...
| **Client (mobile)**    | React Native                                               | Cross-platform mobile development for iOS and Android from a single codebase.                                                                                        |
...
To solve this problem, consider implementing [Chaum-Pedersen protocol/](https://muens.io/chaum-pedersen-protocol/) go
guarantee the encryption consistency.
